...
(function() {
	...
	var SHA256 = C.SHA256 = function(message, options) {
		...
		return options && options.asBytes ? digestbytes : options
				&& options.asString ? Binary.bytesToString(digestbytes) : util
	...
	SHA256._sha256 = function(message) {
		...
		var m = util.bytesToWords(message), l = message.length * 8, H = [
				...
				0x9B05688C, 0x1F83D9AB, 0x5BE0CD19 ], w = [], a, b, c, d, e, f, g, h, i, j, t1, t2;
		...
		for (var i = 0; i < m.length; i += 16) {
			...
			for (var j = 0; j < 64; j++) {
				...
				if (j < 16)
					...
					w[j] = gamma0 + (w[j - 7] >>> 0) + gamma1
							+ (w[j - 16] >>> 0);

				}

				var ch = e & f ^ ~e & g, maj = a & b ^ a & c ^ b & c, sigma0 = ((a << 30) | (a >>> 2))
						^ ((a << 19) | (a >>> 13)) ^ ((a << 10) | (a >>> 22)), sigma1 = ((e << 26) | (e >>> 6))
						^ ((e << 21) | (e >>> 11)) ^ ((e << 7) | (e >>> 25));
